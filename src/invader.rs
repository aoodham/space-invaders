...
impl Invaders {
    ...
    pub fn update(&mut self, delta: Duration) -> bool {
        ...
        let is_downwards = self.get_move();
        if is_downwards {
...
impl Drawable for Invaders {
    fn draw(&self, frame: &mut Frame) {
        for invader in self.army.iter() {
            frame[invader.x][invader.y] = if (self.move_timer.time_left.as_secs_f32() / self.move_timer.duration.as_secs_f32()) > 0.5 { "+" } else { "x" }
        }
    }
}
